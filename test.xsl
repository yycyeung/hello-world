...
<xsl:if test="$DisplayOPACiconsXSLT!='0'">
    ...
    <xsl:if test="string-length(normalize-space($physicalDescription))">
        <span class="results_format">
            <span class="label">; Physical description: </span><xsl:copy-of select="$physicalDescription"></xsl:copy-of>
        </span>
    </xsl:if>
        ...
        <span class="results_literaryform">
            <span class="label"> </span> <!-- Literary text for sound recordings -->
 ...
 <xsl:call-template name="show-lang-041"/>
    ...
    <xsl:if test="$display880">
      <xsl:call-template name="m880Select">
        ...
        <xsl:with-param name="label">Publication: </xsl:with-param>
      </xsl:call-template>
    </xsl:if>
    ...
    <span class="label">Title translated: </span>
         ...
         <xsl:if test="marc:datafield[substring(@tag, 1, 1) = '6' and not(@tag=655)]">
            <span class="results_summary subjects"><span class="label">Topic(s) and subject(s): </span>
